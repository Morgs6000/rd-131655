...
public class LevelRenderer {
    ...
    public void OnLoad() {
        ...
        int centerX = xChunks / 2; // Centro do mapa em X
        int centerZ = zChunks / 2; // Centro do mapa em Z

        float radiusSquared = radius * radius + radius * 0.5f;

        for(int x = 0; x < xChunks; x++) {
            for(int y = 0; y < yChunks; y++) {
                for(int z = 0; z < zChunks; z++) {
                    // Verifica se o chunk está dentro do circulo
                    int dx = x - centerX;
                    int dz = z - centerZ;
